...
%include "lib/sys/dup.asm"
%include "lib/sys/dup2.asm"
...
START:
	...
	test rax, rax
	js dup_fail
	mov r8, rax			; r8: stderr backup

	mov rdi, SYS_STDOUT
	mov rsi, SYS_STDERR
	call dup2			; redirect stderr to stdout
	test rax, rax
	js dup2_fail
	...
	mov rdi, r8
	mov rsi, SYS_STDERR
	call dup2			; recover stderr
	test rax, rax
	js dup2_fail

	mov rsi, .MSG2
	mov rdx, (.MSG_END-.MSG2)
	call write
	cmp rax, rdx
	jnz write_fail
...
.MSG1:
	db `stderr is now stdout\n`
.MSG2:
	db `stderr is now stderr\n`
.MSG_END:
